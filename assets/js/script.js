...
const modalSourceLink = document.querySelector("[data-modal-source]");
...
for (let i = 0; i < testimonialsItem.length; i++) {
  ...
  testimonialsItem[i].addEventListener("click", function () {
    ...
    const source = this.getAttribute("data-testimonials-source");
    const sourceType = this.getAttribute("data-testimonials-source-type");

    if (source && sourceType) {
      modalSourceLink.href = source;

      if (sourceType === "pdf") {
        modalSourceLink.textContent = "📄 View full recommendation (PDF)";
      } else if (sourceType === "linkedin") {
        modalSourceLink.textContent = "🔗 View recommendation on LinkedIn";
      }
